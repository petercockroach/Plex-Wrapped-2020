...
$url = "http://" . $config->ip . ":" . $config->plexpy_port . "/tautulli/api/v2?apikey=" . $config->plexpy_apikey . "&cmd=get_users";
...
// Name
$url = "http://" . $config->ip . ":" . $config->plexpy_port . "

/api/v2?apikey=" . $config->plexpy_apikey . "&cmd=get_user_ips&user_id=" . $id;
$response = json_decode(file_get_contents($url));
...
//MOVIES
$url = "http://" . $config->ip . ":" . $config->plexpy_port . "/tautulli/api/v2?apikey=" . $config->plexpy_apikey . "&cmd=get_history&user_id=" . $id . "&section_id=" . $library_id_movies . "&order_column=full_title&order_dir=asc&length=10000";
$response = json_decode(file_get_contents($url));
...
//SHOWS
$url = "http://" . $config->ip . ":" . $config->plexpy_port . "/tautulli/api/v2?apikey=" . $config->plexpy_apikey . "&cmd=get_history&user_id=" . $id . "&section_id=" . $library_id_shows . "&order_column=full_title&order_dir=asc&length=10000";
$response = json_decode(file_get_contents($url));
$array = $response->response->data->data;
$shows = array();
...
//SHOW BUDDIES
if(count($shows) > 0){
$url = "http://" . $config->ip . ":" . $config->plexpy_port . "/tautulli/api/v2?apikey=" . $config->plexpy_apikey . "&cmd=get_history&section_id=" . $library_id_shows . "&order_column=full_title&order_dir=asc&length=10000&search=" . urlencode($shows[0]["title"]);
$response = json_decode(file_get_contents($url));
$array = $response->response->data->data;
...
//GET USERS LAST 365
$url = "http://" . $config->ip . ":" . $config->plexpy_port . "/tautulli/api/v2?apikey=" . $config->plexpy_apikey . "&cmd=get_plays_by_top_10_users&time_range=365&y_axis=duration";
$response = json_decode(file_get_contents($url));
$top_users = $response->response->data->categories;

//GET TOP MOVIES AND SHOWS
$url = "http://" . $config->ip . ":" . $config->plexpy_port . "/tautulli/api/v2?apikey=" . $config->plexpy_apikey . "&cmd=get_home_stats&time_range=365&stats_type=duration&grouping=1&stats_count=10";
$response = json_decode(file_get_contents($url));
$top_10_movies = array();
$top_10_shows = array();
